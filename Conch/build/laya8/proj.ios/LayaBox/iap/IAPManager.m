...
- (void)failedTransaction:(SKPaymentTransaction *)transaction {
    ...
    self.isProcessing = NO;
    [self.delegate onFailedPurchase:transaction.payment.productIdentifier quantity:transaction.payment.quantity orderID:_order_id result:-1];
}
...
-(void)recharge:(NSString*) jsonParam
{
    ...
    if(jsonParam==NULL)
    {
        ...
        [self.delegate onFailedPurchase:@"nill" quantity:0 orderID:@"nill" result:-4];
        return;
    }
    ...
    if( !pError )
    {
        ...
        if( nNum <= 0 || nil == sProductID || nil == sOrderID || nil == sCallbackURL)
        {
            ...
            [self.delegate onFailedPurchase:sProductID quantity:nNum orderID:sOrderID result:-2 ];
            return;
        }
...
- (void)doVerifyTransaction: (SKPaymentTransaction *)transaction
{
    ...
    if(NULL==transaction.payment.requestData)
    {
        ...
        [[SKPaymentQueue defaultQueue] finishTransaction: transaction];
        [self.delegate onFailedPurchase:@"nill" quantity:0 orderID:@"nill" result:-2];
        return;
    }
    NSMutableDictionary *paymentmulti =(NSMutableDictionary *)[NSKeyedUnarchiver unarchiveObjectWithData:transaction.payment.requestData];//
    NSString* _order_id = [paymentmulti objectForKey:@"order_id"];
        ...
        if (error == nil){
            ...
            if( !pError ){
                ...
                if (nCode == 0){
                    ...
                    self.isProcessing = NO;
                    [self.delegate onSuccessPurchase:transaction.payment.productIdentifier quantity:amount orderID:_order_id];

                }
                else{
                    NSLog(@"[IAP] verify err code = %@ info = %@",resultCode,[pJson objectForKey:@"error-info"]);
                    self.isProcessing = NO;
                    [self.delegate onFailedPurchase:transaction.payment.productIdentifier quantity:amount orderID:_order_id result:-2];
                }
                ...
                [self.delegate onFailedPurchase:transaction.payment.productIdentifier quantity:amount orderID:_order_id result:-4];
            }
            [[SKPaymentQueue defaultQueue] finishTransaction: transaction];
        }
        else{
