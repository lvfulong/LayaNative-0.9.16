...
namespace laya{
    ...
    JCMaterial::~JCMaterial(){
        m_pDisplayRes[0].reset();
        m_pDisplayRes[1].reset();
        m_pTexture[0] = m_pTexture[1] = NULL;
    }

    void JCMaterial::setTexture(int id, const std::shared_ptr<JCDisplayRes>& pDisplayRes) {
        if (id < 2 && id >= 0) {
            m_pDisplayRes[id] = pDisplayRes;
            m_pTexture[id] = pDisplayRes->getTexture();
        }
    }
    ...
    void JCMaterial::setDrawImageShaderParam(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float p_nAlpha, int p_nBlendType){
        m_pDisplayRes[0] = pDisplayRes;
        m_pTexture[0] = pDisplayRes->getTexture();
        m_fAlpha = p_nAlpha;
        setBlendType(p_nBlendType);
        m_bChanged = true;
    }
        void JCMaterial::setFillImageShaderParam(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float p_nAlpha, int p_nBlendType, float nParam1, float nParam2, float nParam3, float nParam4){
        m_pDisplayRes[0] = pDisplayRes;
        m_pTexture[0] = pDisplayRes->getTexture();
        m_fAlpha = p_nAlpha;
        setBlendType(p_nBlendType);
    ...
    void JCMaterial::setDrawFilterImageShaderParam(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float p_nAlpha, int p_nBlendType, float p_nFilterR, float p_nFilterG, float p_nFilterB, float p_nFilterGray){
        m_pDisplayRes[0] = pDisplayRes;
        m_pTexture[0] = pDisplayRes->getTexture();
        m_fAlpha = p_nAlpha;
        setBlendType(p_nBlendType);
	...
	void JCMaterial::setDrawImageShaderParam2(JCTexture* pTexture, float p_nAlpha, int p_nBlendType) {
        m_pDisplayRes[0].reset();
