...
namespace laya
{
    ...
    JCImage::~JCImage()
    {
        if (m_pDisplayRes)
        {
            ...
            {
                JCInAtlasRes* pInAtlasRes = (JCInAtlasRes*)m_pDisplayRes.get();
    ...
    void JCImage::releaseTexture()
    {
        if (m_pDisplayRes)
        {
            ...
            {
                JCTexture* pTexture = (JCTexture*)m_pDisplayRes.get();
    ...
    ENABLE_TYPE JCImage::enable()
    {
        if (m_pDisplayRes.get() == nullptr)
        {
            ...
            {
                m_pDisplayRes =std::shared_ptr<JCDisplayRes>(new JCTexture(m_pTextureManager,TEXTURE_TYPE_NORMAL, m_kBitmapData.m_nTextureWidth, m_kBitmapData.m_nTextureHeight,getFormat()));
                m_pDisplayRes->setRestoreFunction( std::bind( &JCImage::gpuRestoreRes,this,m_pDisplayRes.get() ) );
            }
            else
            {
                m_pDisplayRes = std::shared_ptr<JCDisplayRes>(new JCInAtlasRes());
                m_pDisplayRes->setRestoreFunction(std::bind(&JCImage::gpuRestoreRes, this, m_pDisplayRes.get()));
            }
        }
    ...
    bool JCImage::gpuRestoreRes(JCDisplayRes* pDisplayRes)
    {
        ...
        if (m_pDisplayRes->isInAtlas())
        {
            m_pAltasManager->pushData((JCInAtlasRes*)m_pDisplayRes.get(), &m_kBitmapData );
        }
        else
        {
