...
{
    [[LayaPlatformCallback GetInstance] LP_onMarketInitCallback:@"init" tokenDir:nil];
}
...
-(int)getPayType
{
    return [conchConfig GetInstance]->m_nPayType;
}
-(int)getChargeType
{
    return [conchConfig GetInstance]->m_nChargeType;
}
-(int)getLoginType
{
...
{
    [[LayaPlatformCallback GetInstance] LP_LoginCallback:@"login"];
...
-(void)LP_setRechargeInfo:(NSString*) jsonParam
{
...
-(void)LP_buyProps:(NSString*) jsonParam
{
...
- (void)onSuccessPurchase:(NSString *)p_pProductionIdentifier quantity:(NSInteger)p_iNumber orderID:(NSString *)p_sOrderID
{
    ...
    [[LayaPlatformCallback GetInstance]LP_onRechargeCallback:pJsonString];
    [[LayaPlatformCallback GetInstance]LP_BuyPropsCallback:pJsonString];
}

- (void)onFailedPurchase:(NSString *)p_pProductionIdentifier quantity:(NSInteger)p_iNumber orderID:(NSString *)p_sOrderID
{
    NSString* pJsonString = @"{  \"order_id\" : \"error\",  \"amount\" : \"0\",  \"code\" : \"-2\", \"desc\" : \"error\",  \"product_id\" : \"error\"}";
