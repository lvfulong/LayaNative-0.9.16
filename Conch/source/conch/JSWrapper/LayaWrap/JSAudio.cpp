...
void JSAudio::setSrc( const char* p_sSrc )
{
	...
	if(m_nType==0)
	{
		...
		if( iter == ms_vSaveMp3File.end() )
		{
			...
			//TODO 这里保存了this，可能会有野指针的问题。
			//downloadMgr::getInstance()->download( m_sSrc.c_str(),0,downloadMgr::defProgressFunc,
            //      std::bind( &JSAudio::onDownloaded, this, std::placeholders::_1 ),downloadMgr::defErrorFunc );
		}
		else
		{
	...
	else
	{
		...
		//TODO 这里保存了this，可能会有野指针的问题。
		//downloadMgr::getInstance()->download( m_sSrc.c_str(),0,downloadMgr::defProgressFunc,std::bind( &JSAudio::onDownloaded, this, std::placeholders::_1 ),downloadMgr::defErrorFunc );
	}
}
//------------------------------------------------------------------------------
...
bool JSAudio::onDownloaded(void* p_pRes, std::weak_ptr<int> callbackref)// unsigned long p_nSourceID, const char *p_pszUrl, unsigned char *p_Buff, size_t p_iSize, const char* p_pszFilename )
{
	if( !callbackref.lock() )
		return false;
...
void JSAudio::play()
{
	...
	if( m_nType == 0 )
	{
		...
		{
			JCAudioManager::GetInstance()->playMp3( m_sLocalFileName.c_str(),m_bLoop?-1:0,m_nCurrentTime,this );
		}
		else
		{
			JCAudioManager::GetInstance()->playMp3( m_sSrc.c_str(),m_bLoop?-1:0,m_nCurrentTime,this );
		}
	}
	else if( m_nType == 1 )
	{
		m_pOpenALInfo = JCAudioManager::GetInstance()->playWav( this,m_sSrc );
        if (m_pOpenALInfo)
        {
            JCAudioManager::GetInstance()->setWavVolume(m_pOpenALInfo, m_nVolume);
        }
