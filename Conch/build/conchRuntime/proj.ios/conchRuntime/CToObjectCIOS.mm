...
@end
//-----------------------------------------------------------------
@implementation postCmdToMarketParam
-(id)init:(int)p_nCmd _p1:(const char*)p_sParam;
{
    self = [super init];
    cmd = p_nCmd;
    if( p_sParam )
...
+(void) callMethod:(id)param
{
    ...
    [[conchRuntime GetIOSConchRuntime] callMethod:_param->iObjID
...
std::string CToObjectCGetMarketName()
{
    ...
    {
        return [[[conchRuntime GetIOSConchRuntime]->m_pMarket getMarketName] cStringUsingEncoding:NSUTF8StringEncoding];
    }
    return "";
}
//获得登陆方式
int CToObjectCGetLoginType()
{
    if( [conchRuntime GetIOSConchRuntime]->m_pMarket )
    {
        return [[conchRuntime GetIOSConchRuntime]->m_pMarket getLoginType];
    }
    return 0;
}
//获得平台的类型
int CToObjectCGetEnterPlatformType()
{
    if( [conchRuntime GetIOSConchRuntime]->m_pMarket )
    {
        return [[conchRuntime GetIOSConchRuntime]->m_pMarket getEnterPlatformType];
    }
    return 0;
}
//能否发送到桌面
int CToObjectCMarketCanSendToDesktop( const char* jsonParam )
{
    if( [conchRuntime GetIOSConchRuntime]->m_pMarket )
    {
        NSString* nsJsonParam = [NSString stringWithUTF8String:jsonParam];
        return [[conchRuntime GetIOSConchRuntime]->m_pMarket LP_canSendToDesktop:nsJsonParam];
    }
    return 0;
}
void CToObjectCMarketInit( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_INIT,jsonParam );
}
//平台登录
void CToObjectCMarketLogin( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_LOGIN,jsonParam );
}
//平台切换用户
void CToObjectCMarketSwitchUser( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_SWITCHUSER,jsonParam );
}
//平台登录退出
void CToObjectCMarketLogout( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_LOGOUT,jsonParam );
}
//平台代币充值 sProjectName根据自己需要填写
void CToObjectCMarketUniZFForCoin( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_PAY,jsonParam );
}
//社区
void CToObjectCMarketEnterPlatform( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ENTERPLATFORM,jsonParam );
}
//退出社区
void CToObjectCMarketLeavePlatform( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_LEAVEPLATFORM,jsonParam );
}
//进入游戏，或者进入大厅
void CToObjectCMarketOnGameEvent( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ONGAMEEVENT,jsonParam );
}
//进入BBS
void CToObjectCMarketEnterBBS( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ENTERBBS,jsonParam );
}
//进入游戏反馈
void CToObjectCMarketEnterFeedback( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ENTERFREEBACK,jsonParam );
}
//暂停的处理，当从后台转为前台的时候处理
void CToObjectCMarketOnPause( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ONPAUSE,jsonParam );
}
//退出的处理函数  如果没有处理一定返回false 否则无法退出
bool CToObjectCMarketOnExit( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ONEXIT,jsonParam );
    return true;
}
//充值的时候设置的信息
void CToObjectCMarketSetCZInfo( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_SETCZINFO,jsonParam );
}
//账户管理
void CToObjectCMarketEnterAccountMgr( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ENTERACCOUNTMGR,jsonParam );
}
//购买道具
void CToObjectCMarketBuyProps( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_BUYPROPS,jsonParam );
}
//分享
void CToObjectCMarketEnterShareAndFeed( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ENTERSHAREANDFEED,jsonParam );
}
//facebook的邀请
void CToObjectCMarketEnterInvite( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_ENTERINVITE,jsonParam );
}
//授权
void CToObjectCMarketAuthorize( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_AUTHORIZE,jsonParam );
}
//刷新token
void CToObjectCMarketRefreshToken( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_REFRESHTOKEN,jsonParam );
}
//获得好友列表
void CToObjectCMarketGetGameFriends( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_GETGAMEFRIENDS,jsonParam );
}
//发送桌面图标
void CToObjectCMarketSendToDesktop( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_SENDTODESKTOP,jsonParam );
}
//发送消息给平台
void CToObjectCMarketSendMessageToPlatform( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_SENDMESSAGETOPLATFORM,jsonParam );
}
//打开话题圈
void CToObjectCMarketOpenTopicCircle( const char* jsonParam )
{
    CToObjectCPostMarket( IOS_MARKET_OPENTOPICCIRCLE,jsonParam );
}

void CToObjectCPostMarket(int cmd,const char* sparam )
{
    postCmdToMarketParam* pparam = [[postCmdToMarketParam alloc] init:cmd _p1:sparam];
    [CToObjectCIOS performSelectorOnMainThread:@selector(postCmdToMarket:) withObject:pparam waitUntilDone:NO];
}
void ObjectCOperateMarket(int cmd,std::string sparam )
{
    id<LayaPlatformInterface> pPlatformInterface = [conchRuntime GetIOSConchRuntime]->m_pMarket;
    if( pPlatformInterface )
    {
        NSString* nsValue = [NSString stringWithUTF8String:sparam.c_str()];
        switch (cmd)
        {
        case IOS_MARKET_INIT:[pPlatformInterface LP_Init:nsValue];break;
        case IOS_MARKET_LOGIN: [pPlatformInterface LP_Login:nsValue];break;
        case IOS_MARKET_SWITCHUSER: [pPlatformInterface LP_SwitchUser:nsValue];break;
        case IOS_MARKET_LOGOUT: [pPlatformInterface LP_Logout:nsValue];break;
        case IOS_MARKET_PAY: [pPlatformInterface LP_CZ:nsValue];break;
        case IOS_MARKET_ENTERPLATFORM: [pPlatformInterface LP_EnterPlatform:nsValue];break;
        case IOS_MARKET_LEAVEPLATFORM: [pPlatformInterface LP_LeavePlatform:nsValue];break;
        case IOS_MARKET_ONGAMEEVENT: [pPlatformInterface LP_onGameEvent:nsValue];break;
        case IOS_MARKET_ENTERBBS: [pPlatformInterface LP_enterBBS:nsValue];break;
        case IOS_MARKET_ENTERFREEBACK: [pPlatformInterface LP_enterFeedback:nsValue];break;
        case IOS_MARKET_ONPAUSE: [pPlatformInterface LP_onPause:nsValue];break;
        case IOS_MARKET_ONEXIT: [pPlatformInterface LP_onExit:nsValue];break;
        case IOS_MARKET_SETCZINFO: [pPlatformInterface LP_setCZInfo:nsValue];break;
        case IOS_MARKET_ENTERACCOUNTMGR: [pPlatformInterface LP_enterAccountMgr:nsValue];break;
        case IOS_MARKET_BUYPROPS: [pPlatformInterface LP_gmProps:nsValue];break;
        case IOS_MARKET_ENTERSHAREANDFEED: [pPlatformInterface LP_enterShareAndFeed:nsValue];break;
        case IOS_MARKET_ENTERINVITE: [pPlatformInterface LP_enterInvite:nsValue];break;
        case IOS_MARKET_AUTHORIZE: [pPlatformInterface LP_authorize:nsValue];break;
        case IOS_MARKET_REFRESHTOKEN: [pPlatformInterface LP_RefreshToken:nsValue];break;
        case IOS_MARKET_GETGAMEFRIENDS: [pPlatformInterface LP_getGameFriends:nsValue];break;
        case IOS_MARKET_SENDTODESKTOP: [pPlatformInterface LP_sendToDesktop:nsValue];break;
        case IOS_MARKET_SENDMESSAGETOPLATFORM: [pPlatformInterface LP_sendMessageToPlatform:nsValue];break;
        case IOS_MARKET_OPENTOPICCIRCLE: [pPlatformInterface LP_openTopicCircle:nsValue];break;
        default:
            break;
        }
...
{
    [[conchRuntime GetIOSConchRuntime]->m_pAssistantTouch setHidden:!p_bShow];
}
...
{
    NSLog(@"1 [%i] [%d] [%s] [%s] [%s]",objid,isSync,clsName,methodName,paramStr);

    postCmdToReflectionParam* pParam =
    [[postCmdToReflectionParam alloc] init:objid Class:clsName Method:methodName Param:paramStr];
        
    [CToObjectCIOS performSelectorOnMainThread:@selector(callMethod:) withObject:pParam waitUntilDone:NO];
}
