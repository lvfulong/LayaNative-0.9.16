...
namespace laya 
{
class JSWindowEditBox:public JsObjBase
{
public:
	static JsObjClassInfo JSCLSINFO;
	static void exportJS();

	static JSWindowEditBox* GetInstance();
...
private:
    void _fillText(char* sText, const char* sFont, int nColor, float x, float y, int nTextAlign);

    void _setPosition(int nodeID, float x, float y);

    void _setSize(int nodeID, float w, float h);//test

    void _clearText();

    void _setBgColor(int nodeID, const char* p_sColor);

    void _setAlpha(int nodeID, float alpha);

	enum{oninputid,onkeydownid};
	static	JSWindowEditBox*	ms_pEditBox;
    std::shared_ptr<int>  m_CallbackRef;
    JsObjHandle     m_pJSFunctionOnInput;//JS的回调                                //4
    JsObjHandle     m_pJSFunctionOnKeydown;//JS的回调                              //4
    int m_nNodeID;
    int m_nCaretNode;
    int m_nGraphicsID;
    int m_nSelectNode;
