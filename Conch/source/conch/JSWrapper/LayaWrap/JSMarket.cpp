...
{
    //LOGI(">>>>>>>>>>JSMarket::recharge %s",jsonParam);
	clearConchEngineMemory();
    m_pCZJSFunction.set(czid,this,p_pCallback);
#ifdef ANDROID
	CToJavaBridge::JavaRet kRet;
    CToJavaBridge::GetInstance()->callMethod(CToJavaBridge::JavaClass.c_str(), "recharge", jsonParam, kRet);
#elif __APPLE__
    CToObjectCMarketUniZFForCoin( jsonParam );
#elif WIN32
#endif
}
void JSMarket::onCZEvent( std::string jsonParam )
{
    //LOGI(">>>>>>>>>>JSMarket::onRechargeEvent desc=%s",jsonParam.c_str() );
...
{
    //LOGI(">>>>>>>>>>JSMarket::onRechargeEventCallJS desc=%s",p_sDesc.c_str() );
...
{
    //LOGI(">>>>>>>>>>JSMarket::setRechargeInfo");
    m_pSetCZInfoJSFunction.set(setczinfoid,this,p_pCallback);
#ifdef ANDROID
	CToJavaBridge::JavaRet kRet;
