...
namespace laya
{
    ...
    bool JCWebGLRender::_rendercmd_getProgramParameter( JCMemClass& cmdBuffer )
    {
        ...
        if(CHECK_OUTPUT_DEBUG(pParam->funcID))
        {
            ...
            if(bValidateID)
            {
                sprintf(buf,"%s():fadeID=%d,realID=%d ,name=0x%x,value=%d\n",
                    s_webgl_commands[pParam->funcID],pParam->program,realProgram,pParam->name,value);
            }
            else
            {
                sprintf(buf,"%s():fadeID=%d,name=0x%x,value=%d ,<Invalidate ID!!!>\n",
                    s_webgl_commands[pParam->funcID],pParam->program,pParam->name,value);
            }
            LOGI("%s", buf);
            s_webgl_info.push_back(std::string(buf));
        }
