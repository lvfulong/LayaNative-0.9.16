...
class ConchGraphics implements IBuffer
{
    ...
    */
    ...
    id:number;
    private _direction:number = CanvasRenderingContext2D.DIRECTION['ltr'];
    constructor()
    {
    ...
    fillBorderText(text:string, x:number, y:number, font:string, fillColor:string, borderColor:string, lineWidth:number, textAlign:string,decorationLine:number):void
    {
        if (borderColor)
		{
            font += " " + (lineWidth ? lineWidth : 1) + " " + borderColor;
			if( decorationLine > 0 )
			{
    ...
    drawImageM(img,sx:number,sy:number,sw:number,sh:number,dx:number,dy:number,dw:number,dh:number,mat:any,alpha:number, blendMode:string):void
    {
        ...
        _.wu32(bImage?0:1);
        _.wf32(alpha);
        _.wu32(CanvasRenderingContext2D.BLEND[blendMode] || 0);
        if(mat)
        {
    ...
    setDirection(font: string) {
        this._direction = CanvasRenderingContext2D.DIRECTION[font] || CanvasRenderingContext2D.DIRECTION['ltr'];
        _Buffer.oneInt(this, 29, this._direction); 
    }
    getDirection():string {
        return CanvasRenderingContext2D.DIRECTION[this._direction];
    }
    drawPrimitive(texture:any, l:number, t:number, w:number, h:number, x:number, y:number, vertices:Float32Array, uvs:Float32Array, indices:Uint16Array, mat:any/*Matrix*/, alpha:number, blendMode:string) {
        let vbSize:number = vertices.length;
        let uvSize:number = uvs.length;
        let ibSize:number = indices.length;
        if (vbSize <=0 || uvSize <= 0 || ibSize <=0 || vbSize != uvSize)
        {
            console.error("drawPrimitive warning");
            return;
        }

       
        var _:_Buffer=conch.bf;
        if((vertices as any).conch_id === undefined || !(vertices as any)._upload)
        {
            (vertices as any).conch_id=CanvasRenderingContext2D.ID++;
            (vertices as any).conchRef = new ArrayBufferRef();
            (vertices as any).conchRef.id = (vertices as any).conch_id;
            _.needsz(16);
            _.wu32(0);
            _.wu32(11);
            _.wu32((vertices as any).conch_id);
            _.wu32(vertices.byteLength);
            _.wab(vertices, vertices.byteLength);
            (vertices as any)._upload = true;
        }
        if((uvs as any).conch_id === undefined || !(uvs as any)._upload)
        {
            (uvs as any).conch_id=CanvasRenderingContext2D.ID++;
            (uvs as any).conchRef = new ArrayBufferRef();
            (uvs as any).conchRef.id = (uvs as any).conch_id;
            _.needsz(16);
            _.wu32(0);
            _.wu32(11);
            _.wu32((uvs as any).conch_id);
            _.wu32(uvs.byteLength);
            _.wab(uvs, uvs.byteLength);
            (uvs as any)._upload = true;
        }
        
        if((indices as any).conch_id === undefined || !(indices as any)._upload)
        {
            (indices as any).conch_id=CanvasRenderingContext2D.ID++;
            (indices as any).conchRef = new ArrayBufferRef();
            (indices as any).conchRef.id = (indices as any).conch_id;
            _.needsz(16);
            _.wu32(0);
            _.wu32(11);
            _.wu32((indices as any).conch_id);
            _.wu32(indices.byteLength);
            _.wab(indices, indices.byteLength);
            (indices as any)._upload = true;
        }

        _.needsz(88);
        _.wu32(this.id);
        _.wu32(30);   
        if ((texture as any)._ctx)//HTMLCanvas
        {   
            _.wu32(texture._ctx._nativeObj.conchID);
            _.wu32(0);
        } 
        else 
        {
        ...
        _.wu32((uvs as any).conch_id);
        _.wu32((indices as any).conch_id);
        _.wf32(mat ? mat.a : 1.0);
        _.wf32(mat ? mat.b : 0.0);
        _.wf32(mat ? mat.c : 0.0);
        _.wf32(mat ? mat.d : 1.0);
        _.wf32(mat ? mat.tx : 0.0);
        _.wf32(mat ? mat.ty : 0.0);
        _.wf32(alpha ? alpha : 1.0);
        _.wu32(vbSize / 2);
        _.wu32(indices.byteLength);
    }
}
