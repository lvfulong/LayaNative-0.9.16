...
namespace laya 
{
    ...
    public:
        ...
        JCHtml5Context(JCHtml5Render* pHtml5Render,JCResManager* pTextureManager, JCImageManager* pImageManager, JC2DShaderManager* p2DShaderManager,
                JCHtml5RenderManager* pHtml5RenderManager, JCTextManager* pTextManager,JCFontManager* pFontManager, JCBufferManager* pBufferManager, JCShaderManager* pShaderManager, std::vector<JCICmdDispatch*>* vVectorCmdDispath, JCShapedTextCache* pShapedTextCache);
    ...
    public:
        ...
        */
        ...
        void drawPrimitive(int nImageID, int blendMode, float l, float t, float w, float h, float x, float y, int verticesId, int uvsId, int indicesId, float *matrix, float alpha, int vertexNum, int p_nIBLen);

        void drawPrimitiveCanvas(int nCanvasId, int blendMode, float l, float t, float w, float h, float x, float y, int verticesId, int uvsId, int indicesId, float *matrix, float alpha, int vertexNum, int p_nIBLen);
   ...
   public:
        ...
        void drawTexBlur(const std::shared_ptr<JCDisplayRes>& pDisplayRes,
            float blurW, float blurH, float blur, float blurOffx, float blurOffy,
            unsigned int color);

        void setAtlasCallback(JCConchMesh2D* pConchMesh);

        void xDrawWordRes(JCShapedGlyphSequence* pSequence, int nCountWidth, JCFontInfo* pFont, int x, int y, TEXT_BASE_LINE_TYPE nBaseLineType, TEXT_ALIGN_TYPE nTextAlignType, TextDirection nDirection);

        void flushToRenderTarget();

        void xFillTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, float nParam1, float nParam2, float nParam3, float nParam4, RectGeometry* pGeo, JCMaterial* pMaterial);

        void xFillFilterMatrixTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, float nParam1, float nParam2, float nParam3, float nParam4, JCFilterMatrix& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial);
    private:

        void xDrawImage5(JCImage* pImage, float x, float y, float w, float h, RectGeometry* pGeo, JCMaterial* pMaterial);
        void xDrawTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, RectGeometry* pGeo, JCMaterial* pMaterial);
        void xDrawTexturePixelPerfect(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, RectGeometry* pGeo, JCMaterial* pMaterial);
        void xDrawFilterTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilter& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial);
        void xDrawFilterTexturePixelPerfect(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilter& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial);
        void xDrawFilterMatrixTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilterMatrix& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial);
        void xDrawFilterMatrixTexturePixelPerfect(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilterMatrix& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial);
    ...
    private:
        ...
        std::vector<std::shared_ptr<JCWordRes> > m_vTempWordRes;             ///<临时保存文字用的
        int                     m_nScissorCount = 0;
        std::vector<std::function<void ()> > m_vOnRenderEndFuncs;
        JCShapedTextCache*      m_pShapedTextCache;
