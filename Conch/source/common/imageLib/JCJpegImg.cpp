...
#define LAYA_PROFILE  "LAYA_PROFILE"
    ...
    static boolean ReadLayaProfile(j_decompress_ptr jpeg_info)
    {
        char magick[13] = { 0 };
        memset(magick, 0, 13);
        size_t length;
        BitmapData* bitmap = (BitmapData *)jpeg_info->client_data;
        ...
        if (strcmp(magick, LAYA_PROFILE) != 0)
        {
            /*
            Not a ICC profile, return.
            */
            /*for (int i = 0; i < (int)(length - 12); i++)
            if (GetCharacter(jpeg_info) == EOF)
            break;*/
            return(TRUE);
        }
        int p13 = GetCharacter(jpeg_info);  /* id */
        int p14 = GetCharacter(jpeg_info);  /* markers */

        uint8_t l[4];
        l[0] = GetCharacter(jpeg_info);
        l[1] = GetCharacter(jpeg_info);
        l[2] = GetCharacter(jpeg_info);
        l[3] = GetCharacter(jpeg_info);

        length -= 19;
        if (bitmap->m_pUserData == nullptr)
        {
            int len = l[0] << 24 | l[1] << 16 | l[2] << 8 | l[3];
            bitmap->m_pUserData = new char[len];
        }
        char* p = bitmap->m_pUserData  + 65514L * (p13 - 1);
        for (int i = length - 1; i >= 0; i--)
        {
            int c = GetCharacter(jpeg_info);
            if (c == EOF)
                break;
            *p++ = (unsigned char)c;
        }
	...
	GLOBAL(int) LoadJpegFromMem (BitmapData * texture,unsigned  char * infile,int size )
	{
      ...
      cinfo.client_data = (void *)texture;
      ...
      texture->m_nOffsetX = 0.0f;
      texture->m_nOffsetY = 0.0f;
      texture->m_nTextureWidth = texture->m_nWidth;
      texture->m_nTextureHeight = texture->m_nHeight;
      texture->m_bPremultiplyAlpha = true;//不需要预乘
	  ...
	  //texture->type	= GL_RGBA;
	  texture->m_pImageData = /*std::shared_array<char> */(new char[texture->m_nWidth*texture->m_nHeight*texture->m_nBpp/4]);
