...
namespace laya
{
    ...
    {
#ifdef __APPLE__
        return "ios-conch5-0.9.16.1";
#elif ANDROID
        return "android-conch5-0.9.16.1";
#elif WIN32
        return "window-conch5-0.9.16.1";
#endif
    }
 ...
 int JSConchConfig::getTextShapingMode() const
    {
        return g_kSystemConfig.m_nTextShapingMode;
    }
    void JSConchConfig::setTextCacheNum(int nNum)
    {
        g_kSystemConfig.m_nTextCacheNum = max(nNum, 16);
    }
    int JSConchConfig::getTextCacheNum()
    {
        return g_kSystemConfig.m_nTextCacheNum;
    }
    void JSConchConfig::setTextShapingMode(const char* strMode)
    {
        if (strcmp(strMode, "simple") == 0)
        {
            LOGI("set text shaping mode simple");
            g_kSystemConfig.m_nTextShapingMode = TextShapingMode::Simple;
        }
        else if (strcmp(strMode, "complex") == 0)
        {
        
            LOGI("set text shaping mode complex");
            g_kSystemConfig.m_nTextShapingMode = TextShapingMode::Complex;
        }
        else
        {
            LOGI("invalid text shaping mode %s , set text shaping mode simple", strMode);
            g_kSystemConfig.m_nTextShapingMode = TextShapingMode::Simple;
        }
    }
    void JSConchConfig::exportJS()
    {
        ...
        JSP_ADD_PROPERTY(textCacheNum, JSConchConfig, getTextCacheNum, setTextCacheNum);
        JSP_ADD_PROPERTY(textShapingMode, JSConchConfig, getTextShapingMode, setTextShapingMode);
