...
#define CANVAS_SAVE_CMD_SIZE 1024
    ...
    JCHtml5Context::JCHtml5Context(JCHtml5Render* pHtml5Render, JCResManager* pTextureManager, JCImageManager* pImageManager, JC2DShaderManager* p2DShaderManager, JCHtml5RenderManager* pHtml5RenderManager, JCTextManager* pTextManager, JCFontManager* pFontManager, JCBufferManager* pBufferManager, JCShaderManager* pShaderManager, std::vector<JCICmdDispatch*>* vVectorCmdDispath, JCShapedTextCache* pShapedTextCache)
    {
        ...
        m_pShapedTextCache = pShapedTextCache;
    ...
    void JCHtml5Context::fillRect(float x, float y, float w, float h )
    {
        ...
        case FILL_TYPE_PATTERN:
        {
           ...
           std::shared_ptr<JCDisplayRes> pDisplayRes = pImage->m_pDisplayRes;
           bool bRepeatX = false;
           bool bRepeatY = false;
    ...
    void JCHtml5Context::xFillTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, float nParam1, float nParam2, float nParam3, float nParam4, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_FILLIMAGE));
        pMaterial->setFillImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType, nParam1, nParam2, nParam3, nParam4);
    ...
    void JCHtml5Context::xDrawText(const char* p_sText, float x, float y, char* sFont, int nColor, TEXT_BASE_LINE_TYPE nBaseLineType, TEXT_ALIGN_TYPE nTextAlignType) 
    {
        ...
        int nLen = strlen(p_sText);
        if (nLen < 1)
            return;

        float sx = this->m_pContextData->m_pCurrentContextData->kMatrix.getScaleX();
        float sy = this->m_pContextData->m_pCurrentContextData->kMatrix.getScaleY();
        uint32_t nScale = ceilf(std::max(1.0f, std::max(sx, sy)));

        JCFontInfo* pFont = m_pFontManager->getFontInfoFromText(sFont);

        const JCShapedGlyphSequence& cachedSequence = m_pShapedTextCache->getShapedTextSequence(p_sText, pFont, Direction::ELeftToRight);
        ...
        for (int i = 0, n = cachedSequence.m_vShapedGlyphEntrys.size(); i < n; i++)
        {
            ...
            if (entry.bIsVisible)
            {
                std::shared_ptr<JCWordRes>  pWordRes = m_pTextManager->getWordRes(entry.glyphIndex, pFont, nColor, nScale, entry.pFTFace);
            ...
            else
            {
                m_shapedGlyphSequence.m_nTextWidth += entry.xAdvance;
                m_shapedGlyphSequence.m_vShapedGlyphEntrys.emplace_back(entry);
                m_shapedGlyphSequence.m_vShapedGlyphEntrys.back().pWordRes = nullptr;
            }
        }
        xDrawWordRes(&m_shapedGlyphSequence, m_shapedGlyphSequence.m_nTextWidth, pFont, x, y, nBaseLineType, nTextAlignType, cachedSequence.m_nTextDirection);
    }
    void JCHtml5Context::xDrawWordRes(JCShapedGlyphSequence* pSequence, int nCountWidth, JCFontInfo* pFont, int x, int y, TEXT_BASE_LINE_TYPE nBaseLineType, TEXT_ALIGN_TYPE nTextAlignType, TextDirection nDirection)
    {
        int nOffsetX = 0;
        if (nDirection == RightToLeft)
        {
        ...
        bool bIsUnderlineDrawed = false;
        float nX = x;
        std::vector<JCShapedGlyphEntry>& vShapedGlyphEntry = pSequence->m_vShapedGlyphEntrys;
        for (int i = 0, n = vShapedGlyphEntry.size(); i < n; i++)
        {
            JCShapedGlyphEntry& entry = vShapedGlyphEntry[i];
            std::shared_ptr<JCWordRes> pWordRes = std::static_pointer_cast<JCWordRes>(entry.pWordRes);

            if (entry.bIsVisible)
            {   
                if (pWordRes == nullptr || pWordRes->enable() == ENALBE_TYPE_FAIL)
                {
                    continue;
                }
               
                Rectangle* pRect = pWordRes->getRect();
                float nY = y - nRate * (pWordRes->m_glyphInfo.ascender - pWordRes->m_glyphInfo.descender);
                ...
                float tx = nX + entry.xOffset + pWordRes->m_glyphInfo.xBearing + nOffsetX;
                float ty = nY + (pWordRes->m_glyphInfo.ascender - pWordRes->m_glyphInfo.yBearing) - entry.yOffset;//还没遇到有yOffset的字体
                //LOGI("%d %d %d %d", pWordRes->m_glyphInfo.ascender, pWordRes->m_glyphInfo.yBearing, entry.xOffset, entry.yOffset);
                float widthNoScale = pWordRes->m_glyphInfo.widthNoScale;
                float heightNoScale = pWordRes->m_glyphInfo.heightNoScale;

                //下划线
                if (pFont->m_nDecorationLine == 1 && !bIsUnderlineDrawed)
                {
                    float xStart = x + nOffsetX;
                    float xEnd = xStart + nCountWidth;
                    float yStart = nY + static_cast<int16_t>(pWordRes->m_glyphInfo.ascender) - pWordRes->m_glyphInfo.underlinePos - (int16_t)(0.5f * pWordRes->m_glyphInfo.underlineThickness);
                    this->beginPath();
                    this->setLineWidth(pWordRes->m_glyphInfo.underlineThickness);
                    this->setStrokeStyle(pFont->m_nDecorationColor);
                    this->moveTo(xStart, yStart);
                    this->lineTo(xEnd, yStart);
                    this->stroke();
                    bIsUnderlineDrawed = true;
                }
            
                switch (m_pContextData->m_pCurrentContextData->nFilterType)
                {
                case FILTER_TYPE_NO:
                    xDrawTexturePixelPerfect(pWordRes, tx, ty, widthNoScale, heightNoScale, tx1, ty1, tx2, ty2, m_pGeo, m_pMaterialManager->getMaterial());
                    break;
                case FILTER_TYPE_COLOR:
                    xDrawFilterTexturePixelPerfect(pWordRes, tx, ty, widthNoScale, heightNoScale, tx1, ty1, tx2, ty2, m_pContextData->m_pCurrentContextData->kFilter, m_pGeo, m_pMaterialManager->getMaterial());
                    break;
                case FILTER_TYPE_MATRIX:
                    xDrawFilterMatrixTexturePixelPerfect(pWordRes, tx, ty, widthNoScale, heightNoScale, tx1, ty1, tx2, ty2, m_pContextData->m_pCurrentContextData->kFilterMatrix, m_pGeo, m_pMaterialManager->getMaterial());
                    break;
                default:
                    xDrawTexturePixelPerfect(pWordRes, tx, ty, widthNoScale, heightNoScale, tx1, ty1, tx2, ty2, m_pGeo, m_pMaterialManager->getMaterial());
                    break;
    ...
    void JCHtml5Context::drawParticle(int p_nParticleTemplatid, float p_nX, float p_nY)
    {
        ...
        if (particleTemplate2D!=NULL)
        {
            ...
            pMaterial->setDrawImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType);
    ...
    void JCHtml5Context::drawPrimitiveCanvas(int nCanvasId, int blendMode, float l, float t, float w, float h, float x, float y, int verticesId, int uvsId, int indicesId, float *pMat, float alpha, int vertexNum, int p_nIBLen)
    {
        JCHtml5Render* pSrcHtml5Render = m_pHtml5RenderManager->getHtml5Render(nCanvasId);
        if (pSrcHtml5Render == NULL) return;
        JCHtml5Context* pSrcContext = pSrcHtml5Render->m_pContext;

        JCBufferManager::BufferContent* pib = m_pBufferManager->getBufferById(indicesId);
        JCBufferManager::BufferContent* pvb = m_pBufferManager->getBufferById(verticesId);
        JCBufferManager::BufferContent* puv = m_pBufferManager->getBufferById(uvsId);
        ...
        if (pib == NULL || pvb == NULL || puv == NULL)
            return;
        Rectangle* pRect = pTexture->getRect();

        float rectInAtlas[4];
        rectInAtlas[0] = 0.0f;
        rectInAtlas[1] = 0.0f;
        rectInAtlas[2] = pTexture->m_nWidth / pTexture->m_nTextureWidth;
        rectInAtlas[3] = pTexture->m_nHeight / pTexture->m_nTextureHeight;

        pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_DRAWIMAGE));
        pMaterial->setDrawImageShaderParam2(pTexture, m_pContextData->m_pCurrentContextData->nAlpha * alpha, m_pContextData->m_pCurrentContextData->nBlendType);
        ...
        pMaterial->setBlendType(blendMode);
        JCRenderGroupData* pGroup = m_pDynamicMesh->pushPrimitive(l, t, w, h, JCConchMesh2D::XYUVRGBA, pvb->buffer, puv->buffer, pib->buffer, mymat, pMaterial, rectInAtlas, vertexNum, p_nIBLen, true);
    }

    void JCHtml5Context::drawPrimitive(int nImageID, int blendMode, float l, float t, float w, float h, float x, float y, int verticesId, int uvsId, int indicesId, float *pMat, float alpha, int vertexNum, int p_nIBLen)
    {
        JCBufferManager::BufferContent* pib = m_pBufferManager->getBufferById(indicesId);
        JCBufferManager::BufferContent* pvb = m_pBufferManager->getBufferById(verticesId);
        JCBufferManager::BufferContent* puv = m_pBufferManager->getBufferById(uvsId);
        JCImage* pImage = m_pImageManager->getImage(nImageID);

        if (pImage == NULL || pib == NULL || pvb == NULL || puv == NULL)
            return;

        if (pImage->enable() == ENALBE_TYPE_FAIL)
            return;
        ...
        case FILTER_TYPE_NO:
            pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_DRAWIMAGE));
            pMaterial->setDrawImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha * alpha, m_pContextData->m_pCurrentContextData->nBlendType);
            break;
        case FILTER_TYPE_COLOR:
            pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_DRAWFILTERIMAGE));
            pMaterial->setDrawFilterImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType, m_pContextData->m_pCurrentContextData->kFilter.r, m_pContextData->m_pCurrentContextData->kFilter.g, m_pContextData->m_pCurrentContextData->kFilter.b, m_pContextData->m_pCurrentContextData->kFilter.gray);
            break;
        case FILTER_TYPE_MATRIX:
        ...
        pMaterial->setBlendType(blendMode);
        JCRenderGroupData* pGroup = m_pDynamicMesh->pushPrimitive(l , t, w, h, JCConchMesh2D::XYUVRGBA, pvb->buffer, puv->buffer, pib->buffer, mymat, pMaterial, rectInAtlas, vertexNum, p_nIBLen, false);
    }
    ...
    void JCHtml5Context::xDrawTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        //TEST
    ...
    void JCHtml5Context::xDrawTexturePixelPerfect(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        //TEST
        //pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_BLURIMG));
        //m_pDynamicMesh->pushCmd(std::bind(setBlurParam, this, 274, 341, 10, 1,
    ...
    void JCHtml5Context::xDrawFilterTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilter& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_DRAWFILTERIMAGE));
        pMaterial->setDrawFilterImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType, kFilter.r, kFilter.g, kFilter.b, kFilter.gray);
        pGeo->createForUV(x, y, w, h, tx1, ty1, tx2, ty2);
        applyMatrix(pGeo, /*CLIP_TYPE_NORMAL*/CLIP_TYPE_ONLY_VISIBLE);
        if (pGeo->m_bShow) {
            m_pDynamicMesh->pushElements(JCConchMesh2D::XYUVRGBA, pMaterial,
    ...
    void JCHtml5Context::xDrawFilterTexturePixelPerfect(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilter& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_DRAWFILTERIMAGE));
        pMaterial->setDrawFilterImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType, kFilter.r, kFilter.g, kFilter.b, kFilter.gray);
        pGeo->createForUV(x, y, w, h, tx1, ty1, tx2, ty2);
    ...
    void JCHtml5Context::xDrawFilterMatrixTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilterMatrix& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        m_pDynamicMesh->pushCmd( std::bind(setMatFilterParam, isGpuCanvas() ? this : this->m_pHostGpuContext, m_pContextData->m_pCurrentContextData->kFilterMatrix) );
        pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_MATFILTER));
        pMaterial->setDrawImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType);
    ...
    void JCHtml5Context::xDrawFilterMatrixTexturePixelPerfect(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, JCFilterMatrix& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        m_pDynamicMesh->pushCmd(std::bind(setMatFilterParam, isGpuCanvas() ? this : this->m_pHostGpuContext, m_pContextData->m_pCurrentContextData->kFilterMatrix));
        pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_MATFILTER));
        pMaterial->setDrawImageShaderParam(pDisplayRes, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType);
    ...
    void JCHtml5Context::xFillFilterMatrixTexture(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float x, float y, float w, float h, float tx1, float ty1, float tx2, float ty2, float nParam1, float nParam2, float nParam3, float nParam4, JCFilterMatrix& kFilter, RectGeometry* pGeo, JCMaterial* pMaterial)
    {
        m_pDynamicMesh->pushCmd(std::bind(setMatFilterParam, isGpuCanvas() ? this : this->m_pHostGpuContext, m_pContextData->m_pCurrentContextData->kFilterMatrix));
    ...
    void JCHtml5Context::xDrawRenderTarget(JCHtml5Context* pSrcContext,float sx, float sy, float sw, float sh, float dx, float dy, float dw, float dh)
    {
        ...
        JCTexture* pTexture = pSrcContext->m_pRenderTarget;
        pMaterial->setShaderProgram(m_p2DShaderManager->getShader(SHADER_TYPE_DRAWIMAGE));
        pMaterial->setDrawImageShaderParam2(pTexture, m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType);
        ...
        int nBlurColor, float fBlurValue, float fOffsetX, float fOffsetY)
    {
        ...
        pMaterial->setDrawImageShaderParam2(pTexture->getTexture(), m_pContextData->m_pCurrentContextData->nAlpha, m_pContextData->m_pCurrentContextData->nBlendType);
    ...
    void JCHtml5Context::drawTexBlur(const std::shared_ptr<JCDisplayRes>& pDisplayRes, float blurW, float blurH, float blur, float blurOffx, float blurOffy, unsigned int color){
        float texW = 1;
        float texH = 1;
