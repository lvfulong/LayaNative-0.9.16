...
namespace laya 
{
    ...
    JCRenderGroupData* JCConchMesh::pushPrimitive(float l, float t, float w, float h, int p_nVertType, const char* pVB, const char* pUV, const char* pIB, const Matrix32& pObjMatrix, JCMaterialBase* pMtl, float* pTexTrans, int vertNum, int p_nIBLen, bool flipV) {
        ...
        RectGeometry::VertexInfo* tempBuffer = new RectGeometry::VertexInfo[nVertNum];
        float* f32VB = (float*)pVB;
        float* f32UV = (float*)pUV;
        float rectw = pTexTrans[2] - pTexTrans[0];
        float recth = pTexTrans[3] - pTexTrans[1];
        for (int i = 0; i < nVertNum; i++)
        {
            RectGeometry::VertexInfo& v = tempBuffer[i];
            v.u = l + w * f32UV[2 * i];
            v.v = t + h * f32UV[2 * i + 1];
            if (pObjMatrix.bOnlyTrans)
            { 
                v.x = f32VB[2 * i] + pObjMatrix.m[4];
                v.y = f32VB[2 * i + 1] + pObjMatrix.m[5];
            }
            else
            {
                float& cx = f32VB[2 * i];
                float& cy = f32VB[2 * i + 1];
                float rx = cx * pObjMatrix.m[0] + cy * pObjMatrix.m[2] + pObjMatrix.m[4];
                float ry = cx * pObjMatrix.m[1] + cy * pObjMatrix.m[3] + pObjMatrix.m[5];
                v.x = rx;
                v.y = ry;  
            }
            if (rectw < 1.0f || recth < 1.0f)
            {
                v.u = v.u * rectw + pTexTrans[0];
                v.v = v.v  *recth + pTexTrans[1];
            }
        ...
        JCRenderGroupData* pGroupData = pushElements(p_nVertType, pMtl,
            nVertNum, (void*)(tempBuffer + 0), vblen,
            (unsigned short*)(pIB + 0), p_nIBLen, false);
        delete[] tempBuffer;
        return pGroupData;
    }
