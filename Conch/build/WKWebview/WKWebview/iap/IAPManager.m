...
- (void)failedTransaction:(SKPaymentTransaction *)transaction {
    ...
    self.isProcessing = NO;
    [self.delegate onFailedPurchase:transaction.payment.productIdentifier quantity:transaction.payment.quantity orderID:_order_id];
}
...
-(void)recharge:(NSString*) jsonParam
{
    ...
    if(jsonParam==NULL)
    {
        ...
        [self.delegate onFailedPurchase:@"nill" quantity:0 orderID:@"nill"];
        return;
    }
    ...
    if( !pError )
    {
        ...
        if( nNum <= 0 || nil == sProductID || nil == sOrderID || nil == sCallbackURL)
        {
            ...
            [self.delegate onFailedPurchase:sProductID quantity:nNum orderID:sOrderID];
            return;
        }
        ...
        if (error == nil){
            ...
            if( !pError ){
                ...
                if (nCode == 0){
                    ...
                    self.isProcessing = NO;
                    [self.delegate onFailedPurchase:transaction.payment.productIdentifier quantity:amount orderID:_order_id];
                }
