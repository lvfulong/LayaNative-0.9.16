...
class ConchNodeNew extends ConchNode{
    ...
    static checkexits(fname:string)
    {
...
class ConchLoadingView extends ConchNodeNew{
    private _tips=["新世界的大门即将打开", "敌军还有30秒抵达战场", "妈妈说，心急吃不了热豆腐"];
    private _len=[];
    ...
    _int:any;
    fontstr:string;
    logoNode:ConchNodeNew;//logo节点
    barlen:number;
    barx:number;
    fc:string;
    static IsMV:boolean=true;
    constructor(){
        super();
    ...
    initlen()
    {
        ...
        for(var i:number=0,n=this._tips.length;i<n;i++)
        {
    ...
    createNode(img,x,y,w,h):ConchNodeNew
    {
    ...
    _init(logo:HTMLImageElement)
    {
       ...
       /* 
        var tip=new ConchNodeNew();
        tg=tip._graphics=new ConchGraphics();
        tip.graphics(tg);
        this.tips.addChild(percent);
        this.tips.addChild(tip);*/
        if(!logo)
        {
            n.addChild(this.mc);
        }
    ...
    createMBar(img:HTMLImageElement,l:number,x:number,y:number,w:number,h:number):ConchNodeNew
    {
