   ...
   static public	int 					m_nState=0;		//是否是第一次创建。0 还没创建 ， 1 已经创建， 2被删除了
	...
	public static  void CallBackToJS(Object objOrClass,String methodName,Object ret) {
		...
		if (objOrClass instanceof Class) {
			...
			Integer id = obj2idMap.get(objOrClass);
			if (id == null) {
	...
	public void _showAssistantTouch(final boolean p_bShow )
	{
	    m_Handler.post
        (
            new Runnable()
            {
                ...
                {
					ExportJavaFunction pExportJavaFun = ExportJavaFunction.GetInstance();
					pExportJavaFun.m_pEngine.game_showAssistantTouch(p_bShow);
                }
            }
        );
	}
...
//					if( expjava.m_pEngine.getIsPlug() )
								if (expjava != null)
									expjava.m_pEngine.alertJS("提示", m_sAlertMsg, m_nAlertCallbackType);
//					else
...
//			LayaWrapper.GetInstance().m_LayaEngineContext.setRequestedOrientation(p_nOrientation);
	...
	public static void useChoreographer(int b){
		ExportJavaFunction pExport = ExportJavaFunction.GetInstance();
		if(pExport!=null){
			pExport._useChoreographer(b>0);
		}
	}
