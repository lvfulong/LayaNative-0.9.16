...
namespace laya {
    ...
    void  JCScriptRuntime::init(JCFileResManager* pFileMgr, JCFileSource* pAssetRes, IConchThreadCmdMgr* pThreadCmdSender,JCFreeTypeFontRender* pFreeTypeRender, JCShapedTextCache* pShapedTextCache)
    {
    ...
    void JCScriptRuntime::onThInit(JCEventEmitter::evtPtr evt) 
    {
...
#ifdef JS_V8
        JSObjNode::s_pListJSObj = new simpList();
        if (m_pDbgAgent) 
        {
        ...
        */
        ...
        int nSLoadJS = 0;
        int nJSSize = 0;
        if (m_pAssetsRes->loadFileContent(m_strStartJS.c_str(), sJCBuffer, nJSSize)&& m_pAssetsRes->loadFileContent("scripts/load.js", sLoadJS, nSLoadJS))
        {
...
#ifdef JS_V8
        simpList* pNodeLists = JSObjNode::s_pListJSObj;
        ListNode* pCur = pNodeLists->begin();
        ListNode* pEnd = pNodeLists->end();
        while (pCur != pEnd) {
    ...
    void JCScriptRuntime::setCmdBuffer(JSValueAsParam pArrayBuffer) {
        if (isSupportTypedArrayAPI())
        {
            ...
            if (bIsArrayBuffer)
            {
                m_pCmdBufferSharedWithJS = ab.data;
                m_nCmdBufferSharedWithJSLen = ab.len;
            }
            else {
        ...
        else
        {
            ...
            JSObjectRef arrayObj = JSValueToObject(ctx,pArrayBuffer,NULL);
            JSObjectGetTypedArrayDataNoAlloc(ctx, arrayObj, m_pCmdBufferSharedWithJS, m_nCmdBufferSharedWithJSLen);
#endif
        }
    }
    void JCScriptRuntime::setBuffer(JSValueAsParam pArrayBuffer) {
        if (isSupportTypedArrayAPI())
        {
            ...
            if (bIsArrayBuffer)
            {
                m_pOtherBufferSharedWidthJS = ab.data;
            }
            else {
    ...
    {
        if (m_bIsExit || !m_pJSOnInvalidGLRes.isValid())
            return;
