...
namespace laya
{
    ...
    JCGraphics::JCGraphics(int nID,JCResManager* pTextureManager, JCImageManager* pImageManager, JC2DShaderManager* p2DShaderManager,
        ...
        JCBufferManager* pBufferManager, JCShaderManager* pShaderManager, TEXT_BASE_LINE_TYPE nBaseLineType, JCShapedTextCache* pShapedTextCache): m_pShapedTextCache(pShapedTextCache)
    {
        m_nWordPos = 0;
        ...
        m_nCurrentDirection = Direction::ELeftToRight;
    }
    ...
    void JCGraphics::direction(int nDirection)
    {
        switch (nDirection)
        {
        ...
        case Direction::ERightToLeft:
            m_nCurrentDirection = Direction::ERightToLeft;
            break;
        case Direction::EAuto:
            m_nCurrentDirection =  Direction::EAuto;
            break;
        default:
            LOGE("Invalid direction")
            break;
        }
    }
    ...
    void JCGraphics::fillText(float x, float y, int nTextAlign)
    {
        ...
        JCShapedGlyphSequence* pSequence = &pWordInfo->getWordRes(m_pShapedTextCache, m_pRenderContext, m_pFontManager, m_pTextManager);
        switch (m_nBaseLineType)
        {
        case laya::TBL_TYPE_MIDDLE:
            m_pRenderContext->xDrawWordRes(pSequence, pSequence->m_nTextWidth, pWordInfo->m_pFont, m_nX + x, m_nY + y + pWordInfo->m_pFont->m_nFontSize / 2, TBL_TYPE_MIDDLE, (TEXT_ALIGN_TYPE)nTextAlign, pSequence->m_nTextDirection);
            break;
        default:
            m_pRenderContext->xDrawWordRes(pSequence, pSequence->m_nTextWidth, pWordInfo->m_pFont, m_nX + x, m_nY + y, TBL_TYPE_TOP, (TEXT_ALIGN_TYPE)nTextAlign, pSequence->m_nTextDirection);
            break;
        }
        m_nWordPos++;
    }
    ...
    void JCGraphics::drawImage(int nImageID, float sx, float sy, float sw, float sh, float dx, float dy, float dw, float dh, float alpha,int nType, int blendMode)
    {
        ...
        pCurrentContextData->nBlendType = blendMode;
    ...
    void JCGraphics::drawImageM(int nImageID, float sx, float sy, float sw, float sh, float dx, float dy, float dw, float dh,
        float m11, float m12, float m21, float m22, float m31, float m32, float alpha, int nType, int blendMode)
    {
        m_pRenderContext->save();
        ContextDataInfo* pCurrentContextData = m_pRenderContext->m_pContextData->m_pCurrentContextData;
        pCurrentContextData->nAlpha *= alpha;
    ...
    void JCGraphics::setDrawImageFlag(int nImageID, float sx, float sy, float sw, float sh, float dx, float dy, float dw, float dh,float alpha,int nType, int blendType)
    {
    ...
    {
        m_vWordInfos.push_back( new JCGraphicsWordInfo(m_pFontManager, m_pTextManager, sText, sFont, nColor, m_nCurrentDirection) );
    }
    ...
    void JCGraphics::drawPrimitive(int nImageID, int type, int blendMode, float l, float t, float w, float h,  float x, float y, int verticesId, int uvsId, int indicesId, float *matrix, float alpha, int vertexNum, int p_nIBLen)
    {
        switch (type)
        {
        case 0:
            m_pRenderContext->drawPrimitiveCanvas(nImageID, blendMode, l, t, w, h, m_nX + x, m_nY + y, verticesId, uvsId, indicesId, matrix, alpha, vertexNum, p_nIBLen);
            break;
        case 1:
            m_pRenderContext->drawPrimitive(nImageID, blendMode, l, t, w, h, m_nX + x, m_nY + y, verticesId, uvsId, indicesId, matrix, alpha, vertexNum, p_nIBLen);
            break;
        default:
            LOGE("drawPrimitive unknow type %d", type);
            break;
        }
    }
}
