...
namespace laya
{
	...
	JCConchRender::JCConchRender(JCFileResManager* pFileResManager)
	{
        ...
        m_pShapedTextCache = new JCShapedTextCache(m_pFreeTypeRender);
        m_pCmdDispathManager = new JCCmdDispathManager(m_pTextureManager,m_pImageManager, m_pTextMemoryCanvas, m_p2DShaderManager, m_pWebGLContext,m_pHtml5RenderManager, m_pTextManager, m_pFontManager, m_pBufferManager,m_pShaderManager, m_pShapedTextCache);
        m_pWebGLRender = NULL;
	...
	JCConchRender::~JCConchRender()
	{
        ...
        if (m_pShapedTextCache != NULL)
        {
            delete m_pShapedTextCache;
            m_pShapedTextCache = NULL;
        }
	}
    ...
    void JCConchRender::renderFrame( long p_nCurrentFrame, bool p_bStopRender)
    {
        ...
        */
        ...
        {
            m_pShapedTextCache->garbageCollection();
        }
        //------------------------------------------------------------------------------
        m_kRenderSem.setDataNum(0);
        //性能测试
