...
public class LayaNotifyManager
{
	 ...
	 */
	//-------------------------------------------------------------------
	public static void setRepeatingNotify( int p_nID,int p_nStartTime,int p_nRepeatType,String p_sTickertext,String p_sTitletext,String p_sDesc )
	{
		...
		LayaAlarm.setRepeatingAlarm(LayaConch5.GetInstance().mCtx, p_nID, p_nStartTime, nIntervalTime, p_sTickertext, p_sTitletext, p_sDesc );
	}
	...
	public static void setRepeatWithHourMinute( int p_nID,int p_nStartHour,int p_nStartMinute,int p_nRepeatType,String p_sTickertext,String p_sTitletext,String p_sDesc )
	{
		...
		case ALARM_REPEAT_TYPE_SECOND:
			...
			break;
		case ALARM_REPEAT_TYPE_MINUTE:
			nIntervalTime =60000;// 60 * 1000;
			break;
		case ALARM_REPEAT_TYPE_HOUR:
			nIntervalTime = 3600000;//60 * 60 * 1000;
			break;
		case ALARM_REPEAT_TYPE_DAY:
			nIntervalTime = 86400000;//24 * 60 * 60 * 1000;
			break;
		case ALARM_REPEAT_TYPE_MONTH:
			nIntervalTime = 2592000000l;//30 * 24 * 60 * 60 * 1000;
			break;
		case ALARM_REPEAT_TYPE_YEAR:
			nIntervalTime = 946080000000l;//365 * 30 * 24 * 60 * 60 * 1000;
			break;
		default:
			Log.i( "0","setRepeatingNotify repeattype 错误" );
			break;
	 ...
	 */
	//-------------------------------------------------------------------
	public static void setOnceNotify( int p_nID,int p_nStartTime,String p_sTickertext,String p_sTitletext,String p_sDesc )
	{
		Log.i("0", "==============setOnceNotify id=" + p_nID + ",p_nStartTime=" + p_nStartTime  );
		LayaAlarm.setOnceAlarm( LayaConch5.GetInstance().mCtx, p_nID, p_nStartTime, p_sTickertext, p_sTitletext, p_sDesc );
	}
	/*
	 * 只删除消息，但是定时器还保留
	 */
	...
	public static void removeNotify( int p_nID )
	{
		...
		LayaLocalNotify.removeNotfication( LayaConch5.GetInstance().mCtx,p_nID );
	}
	/*
	 * 只删除全部消息，但是定时器还保留
	 */
	...
	public static void removeAllNotify()
	{
		...
		LayaLocalNotify.removeAllNotification(LayaConch5.GetInstance().mCtx);
	}
	/*
	 * 删除消息
	 * 参数：id
	...
	{
		//删除定时器
		LayaAlarm.deleteAlarm( LayaConch5.GetInstance().mCtx,p_nID );
		//删除推送信息
		removeNotify( p_nID );
	}
	//-------------------------------------------------------------------
