...
class ConchGraphics {
    ...
    fillBorderText(text, x, y, font, fillColor, borderColor, lineWidth, textAlign) {
        if (borderColor)
            font += " " + 1 + " " + borderColor;
        this.fillText(text, x, y, font, fillColor, textAlign);
    }
    ...
    drawImageM(img, sx, sy, sw, sh, dx, dy, dw, dh, mat) {
        var sz = mat ? 68 : 44;
        var fid = mat ? 17 : 16;
        var _ = conch.bf;
        _.needsz(sz);
...
class ConchFps extends ConchNode {
    constructor(color, fontSize, x, y) {
        ...
        this._repaint = false;
        this.fps = this.vertex = this.drawCall = 0;
        this.textAligh = "left";
        this.left = new ConchNode();
        this.addChildAt(this.left, 0);
        var g = new ConchGraphics();
        this._graphics = g;
        g.fillText("FPS", 2, 2, this.fontstr, color, this.textAligh);
        g.fillText("Vertex", 2, fontSize + 10, this.fontstr, color, this.textAligh);
        g.fillText("DrawCall", 2, fontSize * 2 + 20, this.fontstr, color, this.textAligh);
        this.graphics(g);
        ...
        g.fillText("0", 0, 2, this.fontstr, this.color, this.textAligh);
        g.fillText("0", 0, this.fontSize + 10, this.fontstr, this.color, this.textAligh);
        g.fillText("0", 0, this.fontSize * 2 + 20, this.fontstr, this.color, this.textAligh);
        this.setFPSNode();
        this.size(this.fontSize * 7 + 10, this.fontSize * 3 + 30);
    ...
    render(f) {
        ...
        if (this._repaint) {
            var g = this.left._graphics;
            g.clear();
            g.fillText(this.fps + "", 0, 2, this.fontstr, this.color, this.textAligh);
            g.fillText(this.vertex + "", 0, this.fontSize + 10, this.fontstr, this.color, this.textAligh);
            g.fillText(this.drawCall + "", 0, this.fontSize * 2 + 20, this.fontstr, this.color, this.textAligh);
        }
    }
}
...
class _Buffer {
    ...
    wu8(v) {
        this._cmdBuf.setUint8(this._cmdPos[0], v);
        this._cmdPos[0]++;
    }
    wu32(v) {
        this._cmdBuf.setUint32(this._cmdPos[0], v, true);
        this._cmdPos[0] += 4;
    }
    wu16(v) {
        this._cmdBuf.setUint16(this._cmdPos[0], v, true);
        this._cmdPos[0] += 2;
    }
    wf32(v) {
        this._cmdBuf.setFloat32(this._cmdPos[0], v, true);
        this._cmdPos[0] += 4;
    }
    wf32Rect(x, y, w, h) {
        this.wf32(x);
    ...
    wab(arraybuffer, length) {
        if (length > this.ABSIZE / 2) {
            this.flush();
            this._nativeobj.appendBuffer(arraybuffer, length);
            return;
        }
        if (length < 0)
        ...
        var uint8array = new Uint8Array(ab, 0, length);
...
class Navigator {
    ...
    get appVersion() { return 'layabox conch/2.1'; }
    ;
    ...
    var timerqueue = function () {
        ...
        this.update = function () {
            ...
            curAnimFrms.forEach(function (af) {
                if (af.del)
                    return;
                af.func(Date.now() - gTimerQueue.AniFrameStTm);
            });
            this.curtm = Date.now();
            var i = 0;
            var btmq = [];
...
class Location {
    ...
    resolve(fileName) {
        ...
        if (fileName.indexOf("file:///") == 0 || fileName.indexOf("http://") == 0)
            return fileName;
        if ((fileName.charAt(1) == ':' && fileName.charAt(2) == '/'))
            fileName = "file://" + fileName;
        if (fileName.charAt(0) == "/") {
            return this._fullpath + fileName.substring(1, fileName.length);
        }
...
CanvasRenderingContext2D.BLEND = ["source-over", "source-atop", "source-in", "source-out", "destination-over", "destination-atop", "destination-in",
    "destination-out", "lighter", "copy", 'xor', 'normal', 'multiply', 'screen', 'darken', 'lighten',
    'overlay',
    'color-dodge',
    'color-burn',
...
class HTMLCanvasElement extends HTMLElement {
    ...
    onCSS_Transform(mat) {
        ...
        if (sw == 0 || sh == 0) {
            if (mat[4] > mat[5]) {
                alert("设置横竖屏错误! 请在初始html页面meta中设置screenorientation为横屏landscape");
            }
            else {
                alert("设置横竖屏错误! 请在初始html页面meta中设置screenorientation为竖屏portrait");
            }
        }
    }
    toDataURL(type) {
...
window.postMessage = function (data, d) {
    conch.callWebviewJS("window.__getMessemage", encodeURIComponent(JSON.stringify(data)), "");
};
