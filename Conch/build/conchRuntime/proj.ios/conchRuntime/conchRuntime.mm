...
-(void)setConchRuntimeParam:(GLKView*)pGLView EAGLContext:(EAGLContext*) pGLContext downloadThreadNum:(int)nDownloadThreadNum;
{
    ...
    m_pAssistantTouch = [[UIAssistantTouch alloc] initWithParentView:m_pGLKView];
    if([conchConfig GetInstance]->m_bShowAssistantTouch==false)
    {
        [m_pAssistantTouch setHidden:true];
    }
    [self initPlatform];
...
-(void) initPlatform
{
    ...
    {
        m_pMarket = [[LayaPlatformFactory GetInstance] CreateInterface:nsPlatformClassName];
        if( m_pMarket != nil )
        {
            [m_pMarket LP_Init];
        }
...
-(void) initNetworkListener
{
    ...
    m_pNetworkListener=[Reachability reachabilityForInternetConnection];
...
-(void) onGLReady:(int)p_nWidth height:(int)p_nHeight downloadThreadNum:(int)nDownloadThreadNum
{
    if( m_bEngineInited == false )
    {
        ...
        m_pConchEngine = new laya::JCConch(nDownloadThreadNum);
...
-(void) runJsLoop
{
    laya::JSThread* pJSThread = &(laya::JCScriptRuntime::s_JSRT->m_ScriptThread);
    pJSThread->runQueue();
    pJSThread->m_funcLoop();
    if(m_bStopEngine)
    {
        [self reset];
...
-(int)checkNetworkState
{
    ...
    {
        NSLog(@">>>>>>>checkNetworkState wifi");
        return laya::NET_WIFI;
    }
...
-(void) setScreenOrientation:(int)p_nType
{
    ...
    [m_pAssistantTouch onSetScreenOrientation];
}
    ...
    case UIDeviceOrientationPortraitUpsideDown:  // Device oriented vertically, home button on the top
        {
            ...
            [m_pAssistantTouch onSetScreenOrientation];
            [m_pAssistantTouch resetPosition];
        }
        break;
...
-(void)reset
{
    ...
    if(m_pAssistantTouch)
    {
        [m_pAssistantTouch showButtonPanel:false];
        [m_pAssistantTouch resetPosition];
    }
    if (m_pReflection){
...
{
    [m_pReflection callMethod:objid className:cls methodName:method param:param];
}
